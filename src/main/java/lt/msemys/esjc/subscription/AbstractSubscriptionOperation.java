...
import lt.msemys.esjc.proto.EventStoreClientMessages.SubscriptionDropped;
import lt.msemys.esjc.proto.EventStoreClientMessages.UnsubscribeFromStream;
...
import static lt.msemys.esjc.util.Preconditions.*;
...
public abstract class AbstractSubscriptionOperation<T extends Subscription> implements SubscriptionOperation {
    ...
    public void drop(SubscriptionDropReason reason, Exception exception, Channel connection) {
        if (unsubscribed.compareAndSet(false, true)) {
            logger.trace("Subscription {} to {}: closing subscription, reason: {}, exception: {}...",
    ...
    private void action(Runnable action) {
        ...
        if (actionQueue.size() > MAX_QUEUE_SIZE) {
            drop(SubscriptionDropReason.UserInitiated, new Exception("client buffer too big"));
        }
